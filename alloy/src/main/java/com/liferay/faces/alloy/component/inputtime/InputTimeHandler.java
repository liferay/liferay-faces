...
public class InputTimeHandler extends ComponentHandler {
	...
	protected class AutoCompleteMethodRule extends MetaRule {
		...
		public Metadata applyRule(String name, TagAttribute tagAttribute, MetadataTarget metadataTarget) {
			...
			if ((metadataTarget != null) && (metadataTarget.isTargetInstanceOf(InputTime.class))) {

				if (InputTimeRenderer.TIME_SELECT_LISTENER.equals(name)) {
					Method writeMethod = metadataTarget.getWriteMethod(name);
					Class<?>[] args = new Class[] { TimeSelectEvent.class };
