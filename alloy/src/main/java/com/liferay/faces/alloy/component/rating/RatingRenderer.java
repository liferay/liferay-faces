...
public class RatingRenderer extends RatingRendererBase {
	...
	protected void encodeJavaScriptCustom(FacesContext facesContext, UIComponent uiComponent) throws IOException {
		...
		// The only way that the JavaScript ratingObject would have a selectedIndex at this point is if the defaultSelected attribute is set.
		...
		if (selectedIndex.intValue() == -1) {
			...
			// the user has selected their rating, selectedIndex.intValue(), but the rating they selected may have already been rendered as the defaultSelected rating, so
