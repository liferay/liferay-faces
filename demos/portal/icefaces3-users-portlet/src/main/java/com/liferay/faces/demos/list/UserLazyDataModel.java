...
public class UserLazyDataModel extends LazyDataModel<User> {
	...
	public int countRows() {
		...
		try {
			...
			Hits hits = UserLocalServiceUtil.search(companyId, searchCriteria.getFirstName(),
					...
					searchCriteria.getEmailAddress(), status, params, andSearch, QueryUtil.ALL_POS, QueryUtil.ALL_POS,
					sort);
			...
			logger.debug(
				...
				searchCriteria.getScreenName(), searchCriteria.getEmailAddress(), status, andSearch, totalCount);
		}
		catch (Exception e) {
	...
	public List<User> findRows(int startRow, int finishRow) {
		...
		try {
			...
			Hits hits = UserLocalServiceUtil.search(companyId, searchCriteria.getFirstName(),
					searchCriteria.getMiddleName(), searchCriteria.getLastName(), searchCriteria.getScreenName(),
					searchCriteria.getEmailAddress(), status, params, andSearch, startRow, liferayOneRelativeFinishRow,
					sort);
			...
			logger.debug(
				...
				searchCriteria.getFirstName(), searchCriteria.getMiddleName(), searchCriteria.getLastName(),
				searchCriteria.getScreenName(), searchCriteria.getEmailAddress(), status, andSearch, startRow,
