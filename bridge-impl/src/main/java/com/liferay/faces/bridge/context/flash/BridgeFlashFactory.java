...
/**
 * NOTE: PROPOSED-FOR-JSR344-API (FACTORY FOR THE JSF 2.2 API)
 * 
 * See: http://java.net/jira/browse/JAVASERVERFACES_SPEC_PUBLIC-1071
 *
...
public class BridgeFlashFactory {
	...
	static {
		...
		try {
			...
			mojarraGetFlashMethod = mojarraFlashClass.getMethod(MOJARRA_GET_FLASH_METHOD_NAME, new Class[] {});
		}
		catch (Exception e1) {

			try {
				Class<?> myFacesFlashClass = Class.forName(MYFACES_FLASH_FQCN_NEW);
				myFacesGetFlashMethod = myFacesFlashClass.getMethod(MYFACES_GET_FLASH_METHOD_NAME,
