...
public class Icefaces3CompatPortletTest {
	...
	@FindBy(xpath = "//a[contains(@id,'icefaces3compatportlet') and contains(@id,'menuButton')]")
	private WebElement menuButton;
	@FindBy(xpath = "//a[contains(@id,'icefaces3compatportlet') and contains(@id,'menu_preferences')]")
	...
	public void jobApplicant() throws Exception {
		...
		int foo = 0;
		while (!browser.getTitle().contains("ICE3-COMPAT") && foo < 10) {
			
			signIn();
			url = "http://localhost:8080/group/bridge-demos/ice3-compat";
			logger.log(Level.INFO, "browser.navigate().to("+url+")");
			browser.navigate().to(url);
			waitModel(browser);
			Thread.sleep(2000);
